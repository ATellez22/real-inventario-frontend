...
const Taking = () => {
    const txtCantidadRef = useRef<HTMLInputElement | null>(null); // Crea una referencia para el campo de entrada
    const txtCodigoRef = useRef<HTMLInputElement | null>(null);
    const [formSubmitted, setFormSubmitted] = useState(false); //Agrega un estado en tu componente para rastrear si el formulario se ha enviado.

    const formik = useFormik({
        initialValues: {
            txt_usuario: '',
            sel_conteo: '1', //Debe valer 1 para que Formik lo guarde
            txt_ubicacion: '',
            txt_codigo: '',
            txt_descripcion: '',
            txt_cantidad: '',
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                formik.setValues({
                    ...formik.values, // Mantén los valores existentes
                    txt_codigo: '',
                    txt_descripcion: '',
                    txt_cantidad: '',
                });
                ...
                // Maneja errores de envío aquí
            } finally {
                // SetSubmitting se llama incluso en caso de error
                setSubmitting(false);
            }
        }
    });
    ...
    const buscar = async () => { //Buscar articulo
        ...
        if (formik.values.txt_codigo) {
            try {
                ...
                if (txtCantidadRef.current) {
                    (txtCantidadRef.current as HTMLInputElement).focus(); //Foco a txt_cantidad
                }
            ...
            <form id="form" onSubmit={formik.handleSubmit}>
                <div className="bg-white w-96 p-6 rounded shadow-sm">
                    <h3 className="flex flex-col items-center justify-center mb-4 ">
                        <span className="text-gray-700 font-bold text-2xl mb-2">📦 Toma de inventario</span>
                    </h3>

                    <label htmlFor="txt_usuario" className="text-gray-700 font-bold text-lg">Usuario</label>
                    <input
                        type="text"
                    ...
                    <label htmlFor="sel_conteo" className="text-gray-700 font-bold text-lg">Conteo</label>
                    <select
                        id="sel_conteo"
                        className="w-full py-2 bg-gray-200 text-gray-500 px-1 outline-none mb-4 font-bold text-xl text-center uppercase"
                        name="sel_conteo"
                    ...
                    <label htmlFor="txt_ubicacion" className="text-gray-700 font-bold text-lg">Ubicación</label>
                    <input
                        type="text"
                        className="w-full py-2 bg-gray-200 text-gray-500 px-1 outline-none mb-4 font-bold text-xl text-center uppercase"
                        id="txt_ubicacion"
                    ...
                    <label htmlFor="txt_codigo" className="text-gray-700 font-bold text-lg">Código</label>
                    <input
                        type="number"
                        className="w-full py-2 bg-gray-200 text-gray-500 px-1 outline-none mb-4 font-bold text-xl text-center uppercase"
                        id="txt_codigo"
                    ...
                    <label htmlFor="txt_descripcion" className="text-gray-700 font-bold text-lg">Descripción</label>
                    <input
                        type="text"
                        className="w-full py-2 bg-gray-200 text-gray-500 px-1 outline-none mb-4 font-bold text-xl text-center uppercase"
                        id="txt_descripcion"
                    ...
                    <label htmlFor="txt_cantidad" className="text-gray-700 font-bold text-lg">Cantidad</label>
                    <input
                        type="number"
                        className="w-full py-2 bg-gray-200 text-gray-500 px-1 outline-none mb-4 font-bold text-xl text-center uppercase"
                        id="txt_cantidad"
