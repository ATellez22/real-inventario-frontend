...
interface PrimeReactTableProps {
  //Props para buscar las cargas del usuario
  usuario: string;
  conteo: string; 
}

const PrimeReactTable = ({ usuario, conteo }: PrimeReactTableProps) => {
  ...
  const mostrar_tabla = async () => {
    try {
      const response = await fetchApi(
        `inventory/show?txt_usuario=${usuario}&sel_conteo=${conteo}`
      );
  ...
  return (
    <div>
      <Button
        label="Tabla"
        className="font-bold text-lg w-full py-2 rounded transition-colors"
        onClick={() => {
            ...
            <Column
              ...
              body={(rowData) => (
                <div>
                  ...
                  <p>
                    <strong>Descripción:</strong> {rowData.descripcion}
                  </p>
                  <p>
                    <strong>Cantidad:</strong> {rowData.cantidad}
                  </p>
                </div>
              )}
            ></Column>
